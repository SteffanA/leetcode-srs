...
// Functions
    ...
    // Setup for problem table - add Time To Next field, generated by the
    // Time to Next redux
    const ttnProvider = (problem) => {
        if (props.ttn) {
            return new Date(props.ttn[problem._id]).toDateString()
        }
        else {
    ...
    return (
        <div>
            ...
            */}
            ...
            <br/>
            <div>
                <h4>Current List's Problems:</h4>
                <ProblemTable problems={props.curProblems} extraFields={problemTableFields}/>
            </div>
        </div>
    )
}
...
const mapStateToProps = (state) => {
    return {
        ...
        ttn: state.problems.problemIdToTimeToNextSub,
    }
}
