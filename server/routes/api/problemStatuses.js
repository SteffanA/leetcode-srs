...
router.put('/:problem_id', [auth,[
    ...
    try {
        ...
        const problem = await Problem.find({id: req.params.problem_id})
        if (!problem) {
        ...
        if (user.problem_statuses.length !== 0) {
            index = await user.problem_statuses.map(async (status) => {
                //TODO: Need to fix the Model so we don't need to do slow DB lookups here
                fullStatus = await ProblemStatus.findById(status)
                console.log(fullStatus.prob)
                return fullStatus.prob.toString()})
                .indexOf(req.params.problem_id.toString())
        }
        console.log('came out with index ', index)
        if (index !== -1) {
            ...
            // Else TTN is 0, let default handler set next to Date.now

            // Save the problem status
            const updatedStatus = await problem_status.save()
        ...
        // Else problem status for this problem does not exist
            ...
            const problem_status = new ProblemStatus({
                prob: req.params.problem_id,
                interval: (success ? time_multiplier : 1),
            })
            const results = problem_status.results
            if (success) {
                // Increment by current date by interval # of days for next_submission
                problem_status.next_submission = addDays(problem_status.interval)
                results.success += 1
            }
            else {
                results.incorrect += 1
            ...
            console.log(problem_status)

            // Save the problem status
            const newStatus = await problem_status.save()

            // Save problem status to user
            user.problem_statuses.push(newStatus)
            // TODO: Required?
            await user.save()
...
async (req, res) => {
    try {
        ...
        const user = await user.findById(req.user.id)
        const index = user.problem_statuses.map((status) => status.prob.toString()).indexOf(req.params.problem_id.toString())
        ...
        // Check if this 1) Works and 2) Is the best practice
